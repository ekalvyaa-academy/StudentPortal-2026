...
import {
  ...
  initials, debounce, qs, qsa, populateSelect, getTeacherSession, openIdCard
} from './utils.js?v=3';
...
function renderTable() {
    ...
    <tr>
      ...
      <td>
        <div class="table-actions">
          <button class="btn btn-outline btn-icon" data-action="idcard" data-id="${s.StudentID}" title="ID Card"><i class="fa-solid fa-id-card"></i></button>
  ...
  qsa('[data-action="idcard"]', tbody).forEach((btn) =>
    btn.addEventListener('click', () => handlePrintIdCard(btn.dataset.id)));
...
function handlePrintIdCard(studentId) {
  const student = allStudents.find((s) => s.StudentID === studentId);
  if (!student) return;
  openIdCard({
