...
function renderTestsTable() {
  const tbody = qs('#testsTableBody');
  if (!allTests.length) {
    tbody.innerHTML = `<tr><td colspan="9"><div class="empty-state"><i class="fa-solid fa-file-circle-xmark"></i><p>No tests created yet.</p></div></td></tr>`;
    return;
  }
...
async function handleCreateTest(e) {
  ...
  const payload = {
    ...
    chapterNo: qs('#testChapterNo').value.trim(),
    chapterName: qs('#testChapterName').value.trim(),
...
function populateMarksTestDropdown() {
  ...
  allTests.forEach((t) => {
    ...
    opt.value = t.TestID;
    const chapterLabel = t.ChapterName ? (t.ChapterNo ? ` [Ch. ${t.ChapterNo} — ${t.ChapterName}]` : ` [${t.ChapterName}]`) : '';
    opt.textContent = `${t.TestName} — ${t.Subject}${chapterLabel} (${t.Standard} / ${t.Board} / ${t.Batch})`;
...
function renderMarksGrid(test) {
  ...
  area.innerHTML = `
    ...
    ${test.ChapterName ? `<div class="summary-chip" style="margin-bottom:16px; margin-left:8px; display:inline-block;">Chapter: <b>${formatChapter(test)}</b></div>` : ''}
...
function renderStudentReport(res) {
  ...
  const contentHtml = `
    ...
    <table>
      ...
      <tbody>
        ${(marks.recent || []).map((m) => `
          <tr><td>${escapeHtml(m.testName)}</td><td>${escapeHtml(m.subject)}</td><td>${formatChapter({ ChapterNo: m.chapterNo, ChapterName: m.chapterName })}</td><td>${formatDateDisplay(m.date)}</td><td>${escapeHtml(m.marks)} / ${escapeHtml(m.maximumMarks)}</td></tr>
...
function renderTestReport() {
  ...
  const contentHtml = `
    <table>
      <thead><tr><th>Test Name</th><th>Subject</th><th>Chapter</th><th>Standard</th><th>Board</th><th>Batch</th><th>Date</th><th>Max Marks</th></tr></thead>
      <tbody>
        ${allTests.map((t) => `
          <tr>
            <td>${escapeHtml(t.TestName)}</td>
...
// Shared "Ch. 3 — Real Numbers" formatting for a test row, used in the
// Tests table, all printed/downloaded reports, and the student portal.
function formatChapter(t) {
  if (!t || !t.ChapterName) return '-';
  return t.ChapterNo ? `Ch. ${escapeHtml(t.ChapterNo)} — ${escapeHtml(t.ChapterName)}` : escapeHtml(t.ChapterName);
}
